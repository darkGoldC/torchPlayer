...
typedef enum _audio_channel_type_t
{
    ...
    AUDIO_CHANNEL_TYPE_AMBISONICS,//环绕模式
} audio_channel_type_t;

typedef struct _audio_format_t
{
    int nFormat;//音频格式
    unsigned int    nRate;//音频采样率
...
{
    es_format_category_e    nCat;//es 类型
    int                     nCodec;//字幕解码器索引
    ...
    unsigned int    nBitrate;//es流的比特率
    int             nProfile;
    int             nLevel;

    bool            bPacketized;//数据是否是打包的
